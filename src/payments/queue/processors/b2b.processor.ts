...
import { B2BJobData } from '../dto/payment-job.dto';
import { QUEUE_NAMES } from '../queue.constants';
import { B2BPaymentRequest } from '../../providers/mappers/request.mapper';
...
@Processor(QUEUE_NAMES.B2B)
export class B2BProcessor extends WorkerHost {
  constructor(
    ...
    super();
    this.logger.setContext(B2BProcessor.name);
  }

  async process(job: Job<B2BJobData>): Promise<void> {
    const {
      transactionId,
    ...
    const span = createJobSpan('bullmq.b2b.process', traceContext || {}, {
      'job.id': job.id || '',
    ...
    return runWithSpanAsync(span, async () => {
      ...
      try {
        ...
        const response = await this.sasaPayProvider.transferB2B(request);
        ...
        if (!response.success) {
          throw new Error(`B2B transfer failed: ${response.message}`);
        }
        ...
        this.logger.info(
          ...
          'B2B transfer job completed successfully',
        );
      } catch (error) {
  ...
  onCompleted(job: Job<B2BJobData>) {
    this.logger.debug(
      { jobId: job.id, transactionId: job.data.transactionId },
      'B2B transfer job completed event',
    );
  }

  @OnWorkerEvent('failed')
  onFailed(job: Job<B2BJobData>, error: Error) {
    this.logger.error(
      {
        jobId: job.id,
